...
C_values = [0.2 * t for t in range(1, 6)]
...
for C in C_values:
    ...
    print("slack_variable_of_versicolor: {},".format(slackvariables_virginica))
    if c < 5:
        print('-----------------------------------------')
"""
...
for C in C_values:
    c+=1
    setosa = svm.SVC(kernel='linear', C=C, random_state=0)
    versicolor = svm.SVC(kernel='linear', C=C, random_state=0)
    virginica = svm.SVC(kernel='linear', C=C,random_state=0)
